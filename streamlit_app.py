...
st.title("Dr. Z.C.M.")
st.title("Building a prediction model of Horner's syndrome caused by nerve block based on digital intelligence technology")  # 算法名称 and XXX
...
def do_processing():
    ...
    # 准备模型
    gnb = GaussianNB()
    gnb = CalibratedClassifierCV(gnb)

    # 模型训练、显示结果
    with st.spinner("Training, please wait..."):
        model_result = model_fit_score(gnb, X_train, y_train)
    with col1:
        st.text("Training Result")
        msg = model_print(model_result, "GaussianNB - Train")
        st.write(msg)
        # 训练画图
    ...
    with st.spinner("Testing, please wait..."):
        model_test_result = model_score(gnb, X_test, y_test)
    with col2:
        st.text("Testing Result")
        msg = model_print(model_test_result, "GaussianNB - Test")
        st.write(msg)
        # 测试画图
        # fig_test = plt_roc_auc([
        #     (model_test_result, 'GaussianNB',),
        # ], 'Validate ROC')
        # st.pyplot(fig_test)
